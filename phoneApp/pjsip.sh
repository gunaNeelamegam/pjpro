#!/bin/bash	
	...
	cd openh264/  
	make ARCH=x86_64
	sudo  make install DESTDIR=~/Desktop/openh264/   
...
sudo /sbin/ldconfig -v
	 ./configure  --enable-shared --with-external-pa= CFLAGS=-I~/Desktop/usr/local/include/  CDFLAGS=-L~/Desktop/usr/local/lib/ --enable-shared  --with-openh264= CFLAGS=-I~/Desktop/openh264/usr/local/include/wels LDFLAGS=-L~/Desktop/openh264/usr/local/lib
	make dep 
	make lib 
	make
	sudo  make install 
